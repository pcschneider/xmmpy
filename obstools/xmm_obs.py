...
class Obs():
    ...
    def odf_reduction_script(self, fn=None, ofn=None, p2_only=False):
        """
        ...
        if p2_only:
            ifn = os.path.expanduser(Path(script_dir).joinpath("make_xmm_p2.sh"))
            xmmfn = os.path.expanduser(Path(self.config["DATA"]["basedir"]).joinpath("make_xmm_p2.sh"))
        ...
        r+="cd " + os.path.abspath(self.config["DATA"]["basedir"]) + "\n"
        if p2_only:
            r+=". make_xmm_p2.sh "+ self.config["obsID"]
        else:
            r+=". make_xmm.sh "+ os.path.basename(str(fn))
        r+="\ncd ${pwd}"
    ...
    def spec_files(self):
        ...
        if not isinstance(self.config["SPECTRA"]["time_bins"], list) and self.config["SPECTRA"]["time_bins"].lower() == "none":
            for d in dct:
